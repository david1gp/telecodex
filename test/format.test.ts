...
describe("formatTelegramHTML", () => {
  it("formats fenced code blocks with a language", () => {
    const input = "```ts\nconst x = 1 < 2;\n```"
    expect(formatTelegramHTML(input)).toBe('<pre><code class="language-ts">const x = 1 &lt; 2;\n</code></pre>')
  })
  ...
  it("formats inline code with single and double backticks", () => {
    ...
    expect(formatTelegramHTML("Use ``a ` tricky` value`` now")).toBe("Use <code>a ` tricky` value</code> now")
  })
  ...
  it("escapes HTML before applying markdown formatting", () => {
    expect(formatTelegramHTML("<b>not bold</b> & **yes**")).toBe("&lt;b&gt;not bold&lt;/b&gt; &amp; <b>yes</b>")
  })
